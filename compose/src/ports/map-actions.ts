...
import { fontSizeExtractRegexp, fontSizeReplaceRegexp } from './map-style';
import { LayerRef, formatGeoJSON } from './map';
import { Feature as IOFeature, GeometryType } from 'sdi/source';
import queries from '../queries/map';
...
const addSelection =
    (map: Map, localLayersRef: LayerRef[]) => {
        // selection
        ...
        select.on('select', () => {
            if (selectedFeature.getLength() > 0) {
                ...
                layerEvents.editFeature(f.getId());
            }
        });

        return (
            () => {
                const mode = queries.getEditableMode();
                if (mode === 'select') {
...
const addEdit =
    (map: Map) => {
        ...
        infra.setAddHandler((g, f) => {
            layerEvents.addFeature(g);
            map.getLayers().forEach((l) => {
                if (l.get('id') === appQueries.getCurrentLayerId()) {
        ...
        const update = () => {
            const mode = queries.getEditableMode();
            const selected = queries.getEditableSelected();
...
export const addActions =
    (map: Map, localLayersRef: LayerRef[]) => {
        const updateSelect = addSelection(map, localLayersRef);
        const updateModify = addEdit(map);
